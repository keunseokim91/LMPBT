...
#                                            shuffle=True, num_workers=int(opt.workers))
    ...
    train_cifar100 = dset.CIFAR100(root='data', train=True, download=False, transform=transforms.Compose([
                                transforms.Resize((opt.imageSize)),
                                transforms.ToTensor()]))
    dataloader_cifar100 = torch.utils.data.DataLoader(train_cifar100, batch_size=opt.batchSize,
                                                    shuffle=True, num_workers=int(opt.workers))
    ...
    for epoch in range(opt.niter):
        for i, (x, _) in enumerate(dataloader_cifar100):
            x = x.to(device)
            if opt.perturbed:
